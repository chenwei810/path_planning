...
class PlannerRRTStar(Planner):
    ...
    def _get_near_nodes(self, node, radius):
        near_nodes = []
        for n in self.ntree:
    ...
    def planning(self, start, goal, extend_len=None, img=None):
        ...
        for it in range(20000):
            ...
            for node in self.ntree:
                if self.ntree[node] is None:
                    continue

                nearby_nodes = self._get_near_nodes(node, extend_len)

                for n in nearby_nodes:
                    if n == node or self._check_collision(n, node):
                        continue

                    new_cost = self.cost[node] + utils.distance(node, n)
